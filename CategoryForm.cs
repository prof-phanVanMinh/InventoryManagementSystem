    ...
    public partial class CategoryForm : Form
    {
        ...
        private void dgvCategory_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (colName == "Delete")
            {
                ...
                {
                    con.Open();
                    cm = new SqlCommand("SELECT pid FROM tbProduct where catid LIKE '" + dgvCategory.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", con);
                    dr = cm.ExecuteReader();
                    if (dr.Read()==false)
                    {
                        cm.Dispose();
                        ...
                        MessageBox.Show("Category has been productivity!");
